...
# Char-per-token ratio for sizing targets + fallback estimator. 3 splits
# the difference between prose (~4) and dense YAML/HCL/code (~1.5-2).
CHARS_PER_TOKEN = 3
...
def estimate_tokens(text: str) -> int:
    ...
    Uses the real Gemini sentencepiece tokenizer when available; otherwise
    a char-based estimate (`len(text) // CHARS_PER_TOKEN`). Returns 0 for
    empty input. Always >= 1 for non-empty input under the fallback path.
    """
    if not text:
        return 0
    tok = _get_tokenizer()
